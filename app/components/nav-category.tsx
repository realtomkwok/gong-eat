...
export const NavCategory: React.FC<CategoryData> = ({props}) => {
    ...
    return (
        <div className="flex relative w-10/12 items-center h-full">
            ...
            {/*    <Button className={`-translate-x-4 ${!isAtStart ? `visible` : `hidden`}`}*/}
            {/*            icon={{iconName: "chevron_left"}} onClick={handleScroll}/>*/}
            {/*    <Button className={`translate-x-4 ${isAtStart ? `visible` : `hidden`}`}*/}
            {/*            icon={{iconName: "chevron_right"}} onClick={handleScroll}/>*/}
            {/*</div>*/}
        </div>
    )
}